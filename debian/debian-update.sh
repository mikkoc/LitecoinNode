...
then
	...
	then
		...
		rm -f -v $DEBIAN_WEBSITE_DIR/banner.png
		rm -f -v $DEBIAN_WEBSITE_DIR/bootstrap.css
		rm -f -v $DEBIAN_WEBSITE_DIR/favicon.ico
		rm -f -v $DEBIAN_WEBSITE_DIR/style.css
		...
		#Add $DEBIAN_WEBSITE_DIR to the new litecoin-node-status.py script
