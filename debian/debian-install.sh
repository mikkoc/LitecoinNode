...
wget --progress=bar:force -q https://raw.githubusercontent.com/litecoin-foundation/LitecoinNode/master/glob-vars.sh -P /root
	...
	rm -f -v $HOME/$DIST-install-litecoin.sh
		...
		then
			...
			then
				...
				echo "45 23 * * * /usr/bin/bash $HOME/scripts/debian-update.sh" >> /$HOME/scripts/crontempfile
...
if [[ $ANSWER =~ ^([yY])$ ]]
then
	wget --progress=bar:force $DEBIAN_BASE/$DIST-install-statuspage.sh -P $HOME
...
if [[ $ANSWER =~ ^([yY])$ ]]
then
	wget --progress=bar:force $DEBIAN_BASE/$DIST-install-createswap.sh -P $HOME
